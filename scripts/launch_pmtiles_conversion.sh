#!/bin/bash
# Launch EC2 instance to convert GeoTIFFs to PMTiles
...
echo "=========================================="
echo "PMTiles Conversion - EC2 Launcher"
echo "=========================================="
echo ""
echo "Configuration:"
...
SG_ID=$(aws ec2 describe-security-groups --region $REGION \
    --filters "Name=group-name,Values=ontario-satellite-processing" \
...
if aws iam get-instance-profile --instance-profile-name OntarioSatelliteProcessing &>/dev/null; then
...
# Log all output
exec > >(tee -a /var/log/pmtiles-conversion-setup.log)
exec 2>&1

echo "Starting PMTiles conversion setup..."
date

# Update and install dependencies
apt-get update
apt-get install -y python3 python3-pip gdal-bin awscli git
...
mkdir -p /home/ubuntu
aws s3 cp s3://ontario-environmental-data/scripts/convert_to_pmtiles.py /home/ubuntu/ || \
    wget -O /home/ubuntu/convert_to_pmtiles.py https://raw.githubusercontent.com/robertsoden/ontario-environmental-data/main/scripts/convert_to_pmtiles.py

chmod +x /home/ubuntu/convert_to_pmtiles.py
chown ubuntu:ubuntu /home/ubuntu/convert_to_pmtiles.py

# Create status indicator
echo "setup_complete" > /home/ubuntu/status.txt

# Run conversion
echo "Starting PMTiles conversion..."
sudo -u ubuntu python3 /home/ubuntu/convert_to_pmtiles.py > /home/ubuntu/processing.log 2>&1
...
INSTANCE_ID=$(aws ec2 run-instances \
    ...
    --iam-instance-profile Name=OntarioSatelliteProcessing \
    --block-device-mappings '[{"DeviceName":"/dev/sda1","Ebs":{"VolumeSize":200,"VolumeType":"gp3","DeleteOnTermination":true}}]' \
    --user-data "$USER_DATA" \
    --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=ontario-pmtiles-conversion}]' \
...
cat > /tmp/pmtiles_ec2_connection.txt <<EOL
========================================
Ontario PMTiles Conversion Instance
========================================
...
View setup log:
  ssh -i ~/.ssh/ontario-satellite-key.pem ubuntu@$PUBLIC_IP "tail -f /var/log/pmtiles-conversion-setup.log"
...
cat /tmp/pmtiles_ec2_connection.txt
...
echo "Connection details saved to: /tmp/pmtiles_ec2_connection.txt"
echo ""
